...
func TestParseBindPath(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "srcDstMultiple",
			paths: []string{"/opt:/other,/tmp:/other2,"},
			want: []BindPath{
				{
					Source:      "/opt",
					Destination: "/other",
		...
		},
		{
			...
			paths: []string{"/opt:/other:ro,/tmp:/other2:ro"},
			want: []BindPath{
				{
					Source:      "/opt",
					Destination: "/other",
		...
		},
		{
			...
			paths: []string{"/opt:/other:ro,rw,/tmp:/other2:ro,rw"},
			want: []BindPath{
				{
					Source:      "/opt",
					Destination: "/other",
		...
		{
			name:  "srcDstImageSrc",
			paths: []string{"test.sif:/other:image-src=/opt"},
			want: []BindPath{
				{
					Source:      "test.sif",
					Destination: "/other",
		...
		},
		{
			...
			paths:   []string{"test.sif:/other:image-src"},
			want:    []BindPath{},
			wantErr: true,
		},
		{
			name:  "srcDstId",
			paths: []string{"test.sif:/other:image-src=/opt,id=2"},
			want: []BindPath{
				{
					Source:      "test.sif",
					Destination: "/other",
		...
		{
			name:    "invalidOption",
			paths:   []string{"/opt:/other:invalid"},
			want:    []BindPath{},
			wantErr: true,
		},
		{
			name:    "invalidSpec",
			paths:   []string{"/opt:/other:rw:invalid"},
			want:    []BindPath{},
			wantErr: true,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := ParseBindPath(tt.paths)
